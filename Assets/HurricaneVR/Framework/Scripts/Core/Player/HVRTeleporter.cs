...
namespace HurricaneVR.Framework.Core.Player
{
    public class HVRTeleporter : MonoBehaviour
    {
        ...
        protected virtual bool CheckCapsuleFits(Vector3 destination)
        {
            ...
            return Physics.OverlapCapsuleNonAlloc(p1, p2, radius, _dummy, ~0 & ~PlayerFitIgnoreLayerMask, QueryTriggerInteraction.Ignore) == 0;
        }
        ...
        protected virtual void GetCapsuleData(out Vector3 p1, out Vector3 p2, out float radius)
        {
            ...
            p1 = center + Vector3.up * distanceToSphereCenter;
            p2 = center - Vector3.up * distanceToSphereCenter;
        }
        ...
        protected virtual void OnValidTeleport()
        {
            if (Dash)
            {
                StartCoroutine(DashTeleportRoutine());
            }
            else
            {
        ...
        protected virtual IEnumerator DashTeleportRoutine()
        {
            try
            {
                IsTeleporting = true;
                ...
                while (Vector3.Distance(CharacterController.transform.position, TeleportDestination) > .3)
                {
